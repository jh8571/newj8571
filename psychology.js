document.addEventListener('DOMContentLoaded', () => {
    ...
    const testMeta = {
        1: { icon: 'fa-cloud-rain', color: '#6366f1', label: 'PHQ-9 우울척도', badge: '임상 검증' },
        2: { icon: 'fa-wind', color: '#f59e0b', label: 'PSS-10 스트레스척도', badge: '글로벌 표준' },
        3: { icon: 'fa-heart', color: '#ec4899', label: 'RSES 자존감척도', badge: '국제 표준' },
        4: { icon: 'fa-bolt', color: '#10b981', label: 'GAD-7 불안척도', badge: '임상 검증' },
        5: { icon: 'fa-moon', color: '#8b5cf6', label: 'PSQI 수면척도', badge: '국제 표준' }
    };
    ...
    function renderTestList() {
        ...
        testData.forEach(test => {
            const meta = testMeta[test.id] || { icon: 'fa-brain', color: 'var(--accent-color)', label: '심리 테스트', badge: '전문' };
            ...
            card.innerHTML = `
                ...
                <div style="display:flex; align-items:center; gap:8px; margin-bottom:10px;">
                    <span style="font-size:0.7rem; background:${meta.color}22; color:${meta.color}; padding:3px 10px; border-radius:20px; font-weight:800;">${meta.badge}</span>
                </div>
                ...
                <div style="display:flex; align-items:center; gap:10px; margin-bottom:20px; font-size:0.82rem; color:var(--text-muted);">
                    <span><i class="fas fa-list-ol"></i> ${test.questions.length}문항</span>
                    <span><i class="fas fa-clock"></i> 약 ${Math.ceil(test.questions.length / 3)}분</span>
                </div>
    ...
    function getResultInfo() {
        ...
        return { text: '결과를 분석하는 중입니다.', range: '0-0', min: 0, max: 0 };
    }
    ...
    function getScoreLevel(id, score) {
        if (id === 1) { // PHQ-9
            if (score <= 4) return { label: '정상', color: '#10b981', pct: (score / 27) * 100 };
            if (score <= 9) return { label: '경미한 우울', color: '#f59e0b', pct: (score / 27) * 100 };
            if (score <= 14) return { label: '중간 우울', color: '#f97316', pct: (score / 27) * 100 };
            if (score <= 19) return { label: '심한 우울', color: '#ef4444', pct: (score / 27) * 100 };
            return { label: '매우 심한 우울', color: '#991b1b', pct: (score / 27) * 100 };
        }
        if (id === 2) { // PSS-10
            if (score <= 12) return { label: '낮은 스트레스', color: '#10b981', pct: (score / 40) * 100 };
            if (score <= 18) return { label: '적정 스트레스', color: '#f59e0b', pct: (score / 40) * 100 };
            if (score <= 26) return { label: '높은 스트레스', color: '#f97316', pct: (score / 40) * 100 };
            return { label: '만성 스트레스 위험', color: '#ef4444', pct: (score / 40) * 100 };
        }
        if (id === 3) { // RSES
            if (score >= 25) return { label: '높은 자존감', color: '#10b981', pct: (score / 30) * 100 };
            if (score >= 16) return { label: '보통 자존감', color: '#f59e0b', pct: (score / 30) * 100 };
            return { label: '낮은 자존감', color: '#ef4444', pct: (score / 30) * 100 };
        }
        if (id === 4) { // GAD-7
            if (score <= 4) return { label: '정상', color: '#10b981', pct: (score / 21) * 100 };
            if (score <= 9) return { label: '경미한 불안', color: '#f59e0b', pct: (score / 21) * 100 };
            if (score <= 14) return { label: '중간 불안', color: '#f97316', pct: (score / 21) * 100 };
            return { label: '심한 불안', color: '#ef4444', pct: (score / 21) * 100 };
        }
        if (id === 5) { // PSQI
            if (score <= 7) return { label: '양호한 수면', color: '#10b981', pct: (score / 30) * 100 };
            if (score <= 15) return { label: '수면 질 저하', color: '#f59e0b', pct: (score / 30) * 100 };
            if (score <= 23) return { label: '수면 장애 가능성', color: '#f97316', pct: (score / 30) * 100 };
            return { label: '심각한 수면 부족', color: '#ef4444', pct: (score / 30) * 100 };
        }
        const maxSc = getMaxScore();
        return { label: '분석 완료', color: '#6366f1', pct: (score / maxSc) * 100 };
    }
    ...
    function getCopingTips(id, score) {
        ...
        const t = tips[id];
        if (!t) return ['꾸준한 자기 관리와 주기적인 체크가 중요합니다.'];

        const pct = (score / getMaxScore()) * 100;
        if (pct <= 33) return t.low;
        if (pct <= 66) return t.mid;
        return t.high;
    }
        ...
        modalContent.innerHTML = `
            <div style="padding:10px 0;">
                ...
                <div style="text-align:center; margin-bottom:30px;">
                    ...
                    <h2 style="font-size:1.8rem; font-weight:900; color:var(--primary-color);">심리 분석 리포트</h2>
                ...
                <div style="background:var(--bg-color); padding:25px; border-radius:20px; margin-bottom:20px; text-align:center;">
                    ...
                    <div style="font-size:0.85rem; color:var(--text-muted); margin-bottom:15px;">/ ${maxSc}점</div>
                ...
                <div style="background:linear-gradient(135deg, ${level.color}15, ${level.color}05); border:1px solid ${level.color}33;
                    ...
                    <h3 style="color:${level.color}; margin-bottom:12px; font-size:1rem;"><i class="fas fa-quote-left"></i> 전문가 해석</h3>
                ...
                <div style="background:var(--bg-color); padding:25px; border-radius:20px; margin-bottom:20px; border:1px solid var(--border-color);">
                    <h3 style="color:var(--primary-color); margin-bottom:15px; font-size:1rem;"><i class="fas fa-lightbulb"></i> 맞춤형 대처 전략</h3>
                ...
                <div style="background:#fff7ed; border:1px solid #fed7aa; padding:15px 20px; border-radius:14px; margin-bottom:25px;">
                    <p style="font-size:0.8rem; color:#92400e; line-height:1.6;">
                        ⚠️ <strong>안내:</strong> 이 검사는 전문적 임상 진단을 대체하지 않습니다. 증상이 지속되거나 일상에 지장을 줄 경우, 반드시 전문 의료기관을 방문하세요.
                        <strong>정신건강 위기상담전화: 1577-0199</strong> (24시간 운영)
                    </p>
                </div>

                <div style="text-align:center;">
                    <button class="luxury-btn" onclick="location.reload()" style="margin-bottom:15px; max-width:300px;">다른 테스트 하기</button>
                    ${window.getShareUI ? window.getShareUI(`${currentTest.title} 결과: ${level.label}`, 'VitalRest에서 제 마음의 건강 상태를 체크해봤어요. 당신도 한번 확인해보세요!') : ''}
                </div>
            </div>
        `;
    }
